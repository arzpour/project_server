...
const removeBlogThumbnails = async () => {
  try {
    const blogThumbnail = await readdir(
      join(__dirname, "../public/images/blogs/thumbnails")
    );
    ...
    await Promise.all(
      blogThumbnail.map(async (thumbnail) => {
        await unlink(
          join(__dirname, "../public/images/blogs/thumbnails", thumbnail)
        );
      })
    );

    console.log("blog thumbnail removed successfully");
  } catch (error) {
    console.log(error);
  } finally {
    process.exit(1);
...
if (option === "--remove-blog-thumbnails") {
  removeBlogThumbnails();
}
